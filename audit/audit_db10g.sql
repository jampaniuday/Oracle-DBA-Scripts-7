...
spool db_audit10g_&&timestamp&&suffix.log
prompt This script should be run by SYS.
#audit_db10g.sql
...
select dbms_flashback.get_system_change_number as " Current SCN" from dual;
prompt 
prompt
...
select substr(to_char(sysdate,'fmMonth DD, YYYY HH:MI:SS P.M.'),1,35) as "Audit Date" from dual; 
...
select 'Auto Archive Mode : '||decode(value,'TRUE','ENABLED','DISABLED') as "-" from v$parameter
 where name = 'log_archive_start';
select 'Database Uptime    : '||floor(xx)||' days '||floor((xx-floor(xx))*24)||' hours '||
	round(((xx-floor(xx)*24)-floor((xx-floor(xx)*24)))*60)||
       ' minutes' "-" from (select (sysdate-startup_time) xx from v$instance);
...
prompt =======================================================================================
prompt Host Resource Information
prompt =======================================================================================
prompt
prompt =======================================================================================
prompt File system usage:
prompt =======================================================================================
select resource_name, mount_location, mount_options from mgmt$os_fs_mount;

prompt
prompt =======================================================================================
prompt OS and Host Summary:
prompt =======================================================================================
select * from mgmt$os_summary;

prompt
prompt =======================================================================================
prompt OS and Host Resources:
prompt =======================================================================================
select  host_name, os_summary, system_config, MA, cpu_count, vendor_name, os_vendor, distributor_version
from mgmt$os_hw_summary;

prompt
prompt =======================================================================================
prompt OS Kernel configurations:
prompt =======================================================================================
select * from mgmt$os_kernel_params;

prompt
prompt =======================================================================================
prompt Target Information:
prompt =======================================================================================
...
prompt
prompt =======================================================================================
prompt Target Properties:
prompt =======================================================================================
select * from mgmt$target_properties;

prompt
prompt =======================================================================================
prompt Target Oracle Homes:
prompt =======================================================================================
prompt Oracle Home
...
BEGIN
  select sum(bytes) into v_total from dba_data_files;
  select sum(bytes) into v_free  from dba_free_space;
  select sum(bytes) into v_used  from dba_segments;

  v_used_pct := round(v_used/v_total*100,2);
  v_free_pct := round(v_free/v_total*100,2);
  
  DBMS_OUTPUT.PUT_LINE('Used        : '||to_char(v_used, '999,999,999,999,999')||' bytes or '||to_char(v_used_pct,'99.99')||'%');
  DBMS_OUTPUT.PUT_LINE('Free        : '||to_char(v_free, '999,999,999,999,999')||' bytes or '||to_char(v_free_pct,'99.99')||'%');
  DBMS_OUTPUT.PUT_LINE('----------------------------------------');
  DBMS_OUTPUT.PUT_LINE('Total Size  : '||to_char(v_total,'999,999,999,999,999')||' bytes');
END;
/
...
prompt
prompt
prompt Compatible Setting
prompt Note: Once compatible is set to 10.0 or higher, the DB cannot be downgraded.
...
prompt
prompt In 10g, there is no need to set db_file_multiblock_read_count
prompt It is automatically tuned by the RDBMS based on workload.  Unset it.
...
prompt
prompt note: RMAN automatically backs up the spfile.
prompt
prompt Order of Precedence for init and spfiles in decending order of precedence
prompt 1) spfile<SID>.ora
prompt 2) spfile.ora
prompt 3) init<SID>.ora
prompt 4) init.ora
prompt
...
prompt If you set statistics_level=TYPICAL  + SGA_TARGET= 2G (etc)
prompt  then set the following to 0 bytes: db_block_buffers, java_pool, & large_pool
prompt
prompt If not using SGA_TARGET, then the Large pool should 20-30 MB for RMAN to work well.
prompt   NOTE: See RMAN section at the bottom of this output for init parameter caveats
prompt
prompt Flashback Buffer should be about 16 MB (confirm that flashback is turned on in Ent Mgr recovery Section)
prompt   NOTE: If using Flashback, redo buffer must be >= 8M  b/c FB buffer=8 x (log_buffer), and 
prompt   FB buffer needs more space than 8 x 1M.
prompt =======================================================================================
show sga;
select sganame, sgasize from mgmt$db_sga;
prompt
prompt =======================================================================================
prompt in 10g, if the SGA_TARGET is set, below is the correct current SGA component sizing values
prompt =======================================================================================
...
prompt
prompt  In 9i and higher, Non-OLAP, non Budget/Planning 11i instance, olap_page_pool_size is 
...
prompt
prompt =======================================================================================
prompt Database-Level Feature High Water Marks
prompt =======================================================================================
select name, highwater, last_value, description from dba_high_water_mark_statistics;
prompt
prompt
prompt =======================================================================================
...
select name, detected_usages, CURRENTLY_USED, first_usage_date, last_usage_date, description 
from dba_feature_usage_statistics;

prompt
prompt =======================================================================================
prompt Database Feature Usage Dates
prompt =======================================================================================
select name, description, first_usage_date, last_usage_date
...
prompt =======================================================================================
PROMPT                                              
PROMPT Checking if DATABASE VAULT schemas are installed...
prompt =======================================================================================
SELECT DECODE( (Select username from dba_users where UPPER(username)in('DVSYS','DVF')),
 		   'DVSYS', 'DVSYS schema exist', 'DVF', 'DVF schema exist', 
				  'DVSYS and DVF schema does not exist' ) 
FROM dual;
...
prompt =======================================================================================
SELECT DECODE( (Select username from dba_users where UPPER(username)='AVSYS'),
 		   'AVSYS', 'Audit Vault Schema AVSYS exist',  
				'Audit Vault schema does not exist' ) 
FROM dual;
PROMPT
...
prompt =======================================================================================
SELECT DECODE( (Select username from dba_users where UPPER(username)='CONTENT'),
 		   'CONTENT', 'CONTENT schema exist', 
...
prompt
prompt =======================================================================================
prompt Temporary Tablespace Groups
prompt Note: Temp tablespace groups are created to enable true parallelization of large sorts.
prompt =======================================================================================
select group_name, tablespace_name from dba_tablespace_groups;

prompt =======================================================================================
prompt Default Bigfile/Smallfile Tablespace Type
prompt =======================================================================================
...
prompt
prompt =======================================================================================
prompt Undo Tablespace Guarantee (of retention until undo_retention is reached)
prompt note: If UNDO is set to GUARANTEE, and if undo is full, and if a new operation generates 
prompt undo, then it will fail. 
prompt To reverse a guarantee; alter tablespace ~~~ retention noguarantee;
...
prompt    NOTE: If using SBT_TAPE backups, output buffers are part of PGA…input buffers are large_pool (SGA).  
prompt       Make sure that PGA is larger if you use TAPE BU’s.
prompt	     RMAN allocates the tape buffers in the SGA or the PGA, depending on whether I/O slaves are used. 
prompt
prompt 	     If BACKUP_TAPE_IO_SLAVES = true  
prompt 		& LARGE_POOL_SIZE is set, then RMAN allocates tape buffers from the large_pool. 
prompt          & large_pool_size is NOT set then RMAN allocates tape buffers from the shared pool.
prompt
prompt	     If BACKUP_TAPE_IO_SLAVES = false, then RMAN allocates the buffers from the PGA.
prompt
...
prompt "MANUAL" results in "sub-optimal performance and poor PGA memory utilization."
...
prompt =======================================================================================
prompt MTTR Target
prompt In 10g, set FAST_START_MTTR_TARGET=0
prompt This should automanaged by the RDBMS now.
prompt =======================================================================================
...
prompt
prompt =======================================================================================
prompt Aggregated Wait Events by System Component (New to 10g)
prompt =======================================================================================
select a.wait_class, sum(b.time_waited)/1000000 time_waited
from v$event_name a, v$system_event b
where a.name=b.event
...
prompt
prompt =======================================================================================
prompt Is Automatic Storage Management Used?
prompt =======================================================================================
prompt
prompt If there is text below, ASM is in use
show parameters asm_diskgroups;
prompt Aliases for every disk group mounted by the ASM instance
select * from v$asm_alias;
prompt Databases using a disk group managed by the ASM instance
select * from v$asm_client;
prompt Every disk discovered by the ASM instance including those not in any disk group if connected to an ASM instance
select * from v$asm_disk;
prompt ASM disk groups mounted by the local ASM instance.
select * from v$asm_diskgroup;
prompt ASM files in every disk group mounted by the ASM instance.
select * from v$asm_file;
prompt Active ASM long-runningn operations executing in the ASM instance.
...
prompt
prompt =======================================================================================
prompt ASM Instance Information
prompt =======================================================================================
select * from v$asm_client;

prompt
prompt =======================================================================================
prompt ASM Disk Information (ASM)
prompt =======================================================================================
set lines 100
...
select decode(dg.name, null, 'n/a', dg.name) diskgroup, dg.group_number,
       decode(dk.failgroup, null, 'n/a', dk.failgroup) failgroup,
       dk.path,
       dk.label,
       dk.total_mb,
       dk.free_mb avail_mb,
  ...
  from v$asm_disk dk,
       v$asm_diskgroup dg
 where dk.group_number = dg.group_number(+)
order by 1;

prompt
prompt =======================================================================================
prompt ASM Disk Group Usable Free Space
prompt
prompt Note: Allocate space to ASM datafiles based on information below, not query above.
prompt =======================================================================================
prompt ASM Disk Group Free Space
col name format a20 trunc;
set lines 200;
select group_number "DG#", name, block_size, state, total_mb, free_mb
from v$asm_diskgroup;


prompt
prompt =======================================================================================
prompt This shows the hierarchical tree of files stored in the diskgroup
prompt =======================================================================================
...
prompt
prompt =======================================================================================
prompt ASM Install and Redundancy Setups
prompt
prompt States:
prompt   NORMAL = 2 Way Mirroring
prompt   HIGH   = 3 Way Mirroring
prompt   EXTERNAL = Harware-based mirroring like as with an EMC SAN.
prompt =======================================================================================
select state, library from v$asm_disk;


prompt
prompt =======================================================================================
prompt ASM Major Activities:
prompt ASM Operations below will occur if disk group additions or file rebalancing are in progress.
prompt =======================================================================================
select * from v$asm_operation;

prompt
prompt Tablespace containing AUD$ table.  this should be SYSTEM
...
prompt
prompt Currently Active Resource Plans.
select * from v$rsrc_plan;
prompt
prompt Current Activities in the Current Resource Plan
select * from v$rsrc_consumer_group;
prompt 
prompt Current Resource Manager Statistics per Session
select * from v$rsrc_session_info;
prompt
prompt Historical Resource Consumer Group Statistics
select * from v$rsrc_cons_group_history;
prompt
prompt Historical Enabled, Disabled, and Modified Resource Plans for This Instance
...
prompt This query will display passwords for database links..
...
PROMPT DBA_RSRC_PLANS, DBA_rSRC_PLAN_DIRECTIVES, DBA_RULESETS, & DBA_RSRC CONSUMER_GROUPS
prompt =======================================================================================
...
prompt
prompt =======================================================================================
prompt Database Server-Generated Alerts (AWR via OEM)
prompt =======================================================================================
select begin_time, end_time, metric_name, value, metric_unit from v$sysmetric;
prompt
prompt =======================================================================================
prompt Database Server-Generated Alert Thresholds (AWR via OEM)
prompt =======================================================================================
select * from dba_thresholds;
prompt
prompt =======================================================================================
prompt Outstanding Database Server-Generated Alerts (AWR via OEM)
prompt =======================================================================================
select * from dba_outstanding_alerts;
prompt
prompt =======================================================================================
prompt Types of Available Database Server-Generated Alerts
prompt =======================================================================================
...
prompt ==========================================================================================
prompt DBMS_SCHEDULER jobs
prompt NOTE: In 10g, you do not need to set a database stats gather.  This is handled by a preseeded
prompt 	dbms_scheduler program called GATHER_STATS_PROG.
prompt ==========================================================================================
prompt
prompt
col program_name format a30 trunc;
prompt PROGRAMS
SELECT owner, program_name, enabled, comments FROM dba_scheduler_programs; 
prompt JOBS
select owner, job_name,job_type, state, comments from dba_scheduler_jobs;
prompt
prompt
prompt Count of Currently Running Scheduler Jobs
select count(*) from dba_scheduler_running_jobs;
prompt
prompt
prompt Currently Running Scheduler Jobs
select owner,job_name, running_instance, resource_consumer_group from dba_scheduler_running_jobs;
prompt
prompt
prompt Scheduled Jobs that have FAILED in the Last 30 days.
col job_name format a30 trunc;
col job_subname format a20 trunc;
col log_date format a30 trunc;
col owner format a10 trunc;
col client_id format a8 trunc;
col additional_info a10 trunc;
select * from dba_scheduler_job_run_details
where status !='SUCCEEDED'
and log_date > sysdate-30;
prompt
prompt
prompt Scheduled Jobs that have SUCCEEDED in the Last 7 days.
col job_name format a30 trunc;
col job_subname format a20 trunc;
col log_date format a30 trunc;
col owner format a10 trunc;
...
prompt
prompt Job Log History
select job_name, operation, owner from dba_scheduler_job_log;
prompt
prompt

prompt System Job Chains
select * from dba_scheduler_chains;
prompt
prompt

prompt System Job Chain Rules
select * from dba_scheduler_chain_rules;
prompt
prompt

prompt System Job Chain Steps
select * from dba_scheduler_chain_steps;
prompt
prompt

prompt Running System Job Chains
select * from dba_scheduler_running_chains;
prompt
prompt

prompt System Scheduler Job Classes
select * from dba_scheduler_job_classes;
prompt
prompt

prompt System Scheduler Job Windows
select * from dba_scheduler_windows;
prompt
prompt

prompt System Scheduler Job Window Details
select * from dba_scheduler_window_details;
prompt
prompt

prompt System Scheduler Job Window Logs
select * from dba_scheduler_window_log;
prompt

prompt ==========================================================================================
prompt DBMS_JOB jobs
prompt NOTE: In 10g, you do not need to set a database stats gather.  
prompt Checking DBMS_JOB for legacy implementation processing.
prompt These should be moved to DBMS_SCHEDULER programs, schedules and jobs.
prompt ==========================================================================================
set linesize 132
...
prompt
prompt ==================================================================================================
prompt DBA_FGA_AUDIT_TRAIL Information (Fined Grain Auditing via dbms_fga)
prompt Lists all audit policies.
prompt ==================================================================================================
...
prompt
prompt =======================================================================================
prompt Tablespace Bigfiles in Use?
prompt =======================================================================================
...
prompt
prompt =======================================================================================
prompt Directory Objects
prompt Note: Directories must be recreated when a database is upgraded or migrated.
prompt Directory objects are now essential components of the Data Pump functionality.
select * from dba_directories;

prompt
prompt =======================================================================================
prompt Currently Active Data Pump Jobs and Sessions
prompt
...
prompt
prompt =======================================================================================
prompt General Information on File Group Repositories
prompt
...
/
set serveroutput off pagesize 1000
prompt
prompt =======================================================================================
...
prompt You may purge these objects by purging the tablespace they came from:
prompt example: SQL> purge tablesapce ARD;
prompt OR, for a full purge of the whole recylebin:
prompt    
prompt =======================================================================================
prompt Is the Recycle bin enabled? 
prompt Note: It can be turned off by setting     recyclebin=off   in init.ora
show parameters recycl;
SELECT object_name,original_name,type,droptime,dropscn
FROM recyclebin;
prompt To delete ALL contents of the recyclin run this:  "purge dba_recyclebin;"

prompt
prompt =======================================================================================
prompt
PROMPT RMAN-specific Information
prompt
prompt =======================================================================================
prompt
prompt
prompt =======================================================================================
PROMPT DB Incarnation
prompt =======================================================================================
select * from v$database_incarnation;
prompt
prompt
prompt =======================================================================================
PROMPT Trans-Incarnation Resetlogs History
prompt =======================================================================================
select recid, file#, resetlogs_change#, resetlogs_time from v$offline_range;
prompt
prompt
prompt =======================================================================================
PROMPT Trans-Resetlogs Log Sequence History
prompt =======================================================================================
select recid, thread#, sequence#, resetlogs_change#, resetlogs_time from v$log_history;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Recovery Files
prompt =======================================================================================
select * from sys.V_$RECOVER_FILE;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Recovery Log
prompt =======================================================================================
select * from sys.V_$RECOVERY_LOG;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Recovery Status
prompt =======================================================================================
select * from sys.V_$RECOVERY_STATUS;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Recovery File Status
prompt =======================================================================================
select * from sys.V_$RECOVERY_FILE_STATUS;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Recovery Progress
prompt =======================================================================================
select * from sys.V_$RECOVERY_PROGRESS;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Recovery File Desination
prompt =======================================================================================
select * from sys.V_$RECOVERY_FILE_DEST;
prompt
prompt
prompt =======================================================================================
PROMPT DB RMAN Flash Recovery Area Usage
prompt Use Enterprise Manager to enable flashback recovery.
prompt =======================================================================================
select * from sys.V_$FLASH_RECOVERY_AREA_USAGE;
prompt 
prompt Is Flashback Database Enabled?
select flashback_on from v$database;
prompt
prompt Flashback retention (minutes)
select name, value from v$parameter where name='db_flashback_retention_target';
prompt
prompt Current Flashback-able Window
select oldest_flashback_scn, oldest_flashback_time from v$flashback_database_log;
prompt FBA disk quota
select estimated_flashback_size, flashback_size from v$flashback_database_log;
prompt Monitor loggin in the Flashback database logs
select* from v$flashback_database_stat;

prompt
prompt =======================================================================================
PROMPT DB RMAN Block Change Tracking File
prompt warning: RMAN can only store 8 incremental backups of change data.
prompt =======================================================================================
select * from sys.v$block_change_tracking;
PROMPT Block Change Tracking Information:
select filename, status, bytes from v$block_change_tracking;


prompt =======================================================================================
PROMPT DB RMAN Block Change Tracking Performance
prompt Use the data below to determine if BCT is valuable 
PROMPT or whether the AVG change rate is too high for incremental to be efficient.
prompt =======================================================================================
PROMPT Monitor for Incremental Backups:
...
prompt =======================================================================================
PROMPT DB RMAN CONFIGURE settings
prompt Compare the output to these recommendations:
prompt
prompt      CONFIGURE RETENTION POLICY TO RECOVERY WINDOW OF 90 DAYS;
PROMPT		NOTE: use "RECOVERY WINDOW" not "RETENTION".  RETENTION is less recovery-directed.
prompt      CONFIGURE BACKUP OPTIMIZATION ON;
prompt      CONFIGURE DEFAULT DEVICE TYPE TO 'SBT_TAPE';
prompt      CONFIGURE CONTROLFILE AUTOBACKUP ON;
prompt      CONFIGURE CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE DISK TO '%F';
prompt      CONFIGURE CONTROLFILE AUTOBACKUP FORMAT FOR DEVICE TYPE SBT_TAPE TO '%F'; # default
prompt      CONFIGURE DEVICE TYPE DISK PARALLELISM 2;
prompt      CONFIGURE DEVICE TYPE SBT_TAPE PARALLELISM 1; # default
prompt      CONFIGURE DATAFILE BACKUP COPIES FOR DEVICE TYPE DISK TO 1; # default
prompt      CONFIGURE DATAFILE BACKUP COPIES FOR DEVICE TYPE SBT_TAPE TO 1; # default
prompt      CONFIGURE ARCHIVELOG BACKUP COPIES FOR DEVICE TYPE DISK TO 1; # default
prompt      CONFIGURE ARCHIVELOG BACKUP COPIES FOR DEVICE TYPE SBT_TAPE TO 1; # default
prompt      RMAN configuration has no stored or default parameters
prompt      CONFIGURE MAXSETSIZE TO UNLIMITED; # default
prompt      CONFIGURE SNAPSHOT CONTROLFILE NAME TO '/HYPPROD/oracle/product/9.2.0.4_32/dbs/snapcf_HYPPROD.f'; # default
prompt
prompt =======================================================================================
prompt Schedule this routine maintenance to be run quarterly, manually:
prompt  	run {
prompt  	allocate channel ch1 type 'SBT_TAPE' parms="ENV=(NSR_SERVER=bkup1)";
prompt  	crosscheck archivelog all;
prompt  	delete expired archivelog all;
prompt  	crosscheck backup;
prompt  	delete expired backup;
prompt  	delete obsolete;
prompt  	report need backup of 7 days;
prompt  	release channel ch1;
prompt  	}
prompt =======================================================================================
prompt
col name format a40 trunc;
col value format a40 trunc;
set lines 150;
...
prompt
prompt =======================================================================================
prompt RMAN Memory Usage
PROMPT    In order for rman to user parallelism and the large_pool the following must be true, 
prompt    BACKUP_TAPE_IO_SLAVES=TRUE  +  large_pool_size must be set
PROMPT    RMAN allocates the tape buffers in the SGA or the PGA, depending on whether I/O slaves are used. 
prompt
prompt 	     If BACKUP_TAPE_IO_SLAVES = true  
prompt 		& LARGE_POOL_SIZE is set, then RMAN allocates tape buffers from the large_pool. 
...
prompt =======================================================================================
PROMPT
PROMPT BACKUP_TAPE_IO_SLAVES
SHOW PARAMETERS BACKUP_TAPE_IO_SLAVES;
PROMPT LARGE_POOL
SHOW PARAMETERS LARGE_POOL;
PROMPT PGA_AGGREGATE TARGET
SHOW PARAMETERS  PGA_AGGREGATE TARGET;

prompt
prompt =======================================================================================
prompt Controlfile Backup Area Information
prompt 	The controlfile stores information about backups based on the control_file_record_keep_time
prompt		init.ora setting.  Consider running this: "alter system set control_file_record_keep_time=30;"
prompt		or longer if you have no RMAN recovery catalog.
prompt The query below gives you a range of backup information stored in the CF.
prompt  IF you see that "BACKUP DATAFILE" has a larger number of rows (more than a few thousand), 
prompt          then you need to run a crosscheck and delete of obsolete and expired backups.
prompt =======================================================================================
set lines 100;
...
prompt
prompt =======================================================================================
PROMPT Recent Backup Activity 
prompt   NOTE: Tape backups use async IO.  This queries v$backup_async_io.  
prompt 	       If no data found, then tape config is wrong and query v$backup_sync_io
prompt =======================================================================================
col filename format a26 trunc;
set lines 150;
select device_type "Device", type, filename, io_count, ready, short_waits, long_waits, long_waits/io_count,
...
prompt
prompt =======================================================================================
PROMPT DB File Availability and Recovery States
prompt =======================================================================================
@filestats
